...
- `Consumer` processes the event stream with named cursors. `NewConsumer(fn, cfg)` creates a consumer; `Run(ctx)` for polling, `RunRealtime(ctx)` for LISTEN/NOTIFY + polling fallback. Each consumer tracks its own position via `entity_event_consumers` table with TTL-based per-consumer lock.
- `Health(ctx)` returns `HealthStatus` with DB health (ping latency, pool stats), event activity (last event, unpublished count), and consumer status (name, lag, lock holder). `HealthError(ctx)` for simple liveness probes.
